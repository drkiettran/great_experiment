...
# # Get the test scenarios from a JIRA issue by issue key
# Ref: https://jira.readthedocs.io/examples.html
...
class Microservice(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        app_log.info(f'getting test scenarios for issue {issue_id}')
        
        try:
            self.jira_issue = app_jira_handle.issue(issue_id)
            self.response_msg = self.get_success_response(self.jira_issue.fields.labels)
...
app_name = 'ms_jira_get_test_scenarios'
app_jira_handle = None
app_default_server_port = 8484
